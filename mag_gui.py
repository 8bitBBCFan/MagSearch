...
#                     os.system(s)
    ...
    def search(self, event):
        ...
        if entry[0] == '#':
            ...
            if fn != '':
                self.pdf_reader(se.mag_folder + fn, page)
    ...
    def pdf_reader(self, file, page):
        if not os.path.exists(file):
            s = 'Document "{}" is missing'.format(file)
            se.print('\n' + s, 'red')
            self.SetStatus(s)
        else:
            if PLATFORM == 'Linux':
